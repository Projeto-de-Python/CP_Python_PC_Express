...
class MLPredictor:
    ...
    def _get_sales_data(self, product_id: Optional[int] = None, days: int = 90):
        ...
        query = self.db.query(
            ...
            Product.nome,
            Product.categoria
        ).join(Product, SaleItem.produto_id == Product.id).join(
        ...
        df = pd.DataFrame(sales_data, columns=[
            'produto_id', 'quantidade', 'preco_unitario', 'preco_total', 
            'criado_em', 'nome', 'categoria'
        ])
    ...
    def _get_stock_movements(self, product_id: Optional[int] = None, days: int = 90):
        ...
        query = self.db.query(
            ...
            Product.nome
        ).join(Product, StockMovement.produto_id == Product.id).filter(
            and_(
                StockMovement.user_id == self.user_id,
                StockMovement.criado_em >= start_date
            )
        )
        ...
        df = pd.DataFrame(movements_data, columns=[
            'produto_id', 'tipo', 'quantidade_alterada', 'quantidade_resultante',
            'motivo', 'criado_em', 'nome'
        ])
    ...
    def predict_demand(self, product_id: int, days_ahead: int = 30) -> Dict:
        ...
        try:
            ...
            daily_sales = sales_df.groupby('data').agg({
                'quantidade': 'sum',
                'preco_total': 'sum',
                'produto_id': 'count'
            }).reset_index()
            ...
            daily_sales['rolling_mean_7'] = daily_sales['total_quantity'].rolling(7, min_periods=1).mean()
            daily_sales['rolling_mean_14'] = daily_sales['total_quantity'].rolling(14, min_periods=1).mean()
            daily_sales['rolling_std_7'] = daily_sales['total_quantity'].rolling(7, min_periods=1).std()
            ...
            features = ['dia_semana', 'mes', 'lag_1', 'lag_7', 'rolling_mean_7', 'rolling_mean_14', 'rolling_std_7']
            ...
            current_lag_7 = daily_sales['total_quantity'].iloc[-7] if len(daily_sales) >= 7 else current_lag_1
            ...
            for i, date in enumerate(future_dates):
                ...
                if i == 0:
                    ...
                    lag_7 = future_predictions[i-6]['predicted_quantity'] if i >= 6 else current_lag_7
            ...
            return {
                ...
                "avg_daily_demand": sum(p['predicted_quantity'] for p in future_predictions) / len(future_predictions),
                "historical_data_points": len(daily_sales),
                "last_actual_sale": daily_sales['total_quantity'].iloc[-1] if len(daily_sales) > 0 else 0
            }
    ...
    def optimize_price(self, product_id: int) -> Dict:
        ...
        try:
            ...
            price_analysis = sales_df.groupby('preco_unitario').agg({
                'quantidade': ['sum', 'count', 'mean'],
                'preco_total': 'sum'
            }).reset_index()
            
            price_analysis.columns = ['price', 'total_quantity', 'sales_count', 'avg_quantity', 'total_revenue']
            ...
            # Calculate price elasticity
            elasticity = model.coef_[0] * (current_price / current_quantity) if current_quantity > 0 else 0
            
            # Calculate revenue impact
            current_revenue = current_price * current_quantity
            revenue_increase = ((optimal_scenario['revenue'] - current_revenue) / current_revenue * 100) if current_revenue > 0 else 0
            ...
            return {
                "success": False,
                "message": f"Error in price optimization: {str(e)}"
            }
    ...
    def detect_anomalies(self, product_id: Optional[int] = None) -> Dict:
        ...
        try:
            ...
            daily_features = sales_df.groupby('data').agg({
                'quantidade': ['sum', 'count', 'std'],
                'preco_total': ['sum', 'mean']
            }).reset_index()
            
            daily_features.columns = ['data', 'total_quantity', 'sales_count', 'quantity_std', 'total_revenue', 'avg_revenue']
            ...
            features = ['total_quantity', 'sales_count', 'quantity_std', 'total_revenue', 'avg_revenue']
    ...
    def get_stock_optimization(self, product_id: int) -> Dict:
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"Error in stock optimization: {str(e)}"
            }
    
    def _generate_stock_recommendations(self, current_stock: float, reorder_point: float, 
                                      stock_cover_days: float, avg_demand: float) -> List[Dict]:
    ...
    def _generate_ml_recommendations(self, demand_prediction: Dict, price_optimization: Dict, 
                                   anomalies: Dict, stock_optimization: Dict) -> List[Dict]:
